...
namespace local_evalimport_vendor;
...
class SimpleXLS
{
    // Local hardening: bound metadata work and sector traversal.
    private $workBudget = 2000000;
    private function consumeBudget()
    {
        if (--$this->workBudget < 0) {
            throw new \UnexpectedValueException('XLS parsing budget exceeded');
        }
    }
...
/**
    ...
    {
        $this->debug = $debug;
        if (!$this->oleRead($filename, $isData) || !$this->parseEntries()) {
            throw new \UnexpectedValueException('Invalid XLS workbook');
        }
    }
    ...
    public function rowsEx($sheetNum = 0, $limit = 0)
    {
        if ($this->sheets[ $sheetNum ]) {
            $s      = $this->sheets[ $sheetNum ];
            $result = array();
            for ($i = 0; $i < $s['numRows']; $i ++) {
                ...
                for ($j = 0; $j < $s['numCols']; $j ++) {
                    $this->consumeBudget();
                    $v = isset($s['cellsInfo'][$i][$j]) ? $s['cellsInfo'][$i][$j] : array();
//                    if ( $v['type'] === self::TYPE_RK || $v['type'] === self::TYPE_RK2 ||
                    $v['value'] = isset($s['cells'][$i][$j]) ? $s['cells'][$i][$j] : '';
                    $r[ $j ] = $v;
                }
                $result[] = $r;
                $limit--;
    ...
    protected function oleRead($sFileName, $isData = false)
    {
        ...
        if ($numBigBlockDepotBlocks > strlen($this->data) / self::BIG_BLOCK_SIZE) {
            throw new \UnexpectedValueException('Invalid XLS FAT size');
        }
        ...
        if ($numExtensionBlocks > strlen($this->data) / self::BIG_BLOCK_SIZE) {
            throw new \UnexpectedValueException('Invalid XLS DIFAT size');
        }
...
*/
        ...
        while ($sbdBlock !== - 2) {
            ...
            if (isset($visited[$sbdBlock]) || $sbdBlock < 0) {

                throw new \UnexpectedValueException('Invalid XLS sector chain');

            }

            $visited[$sbdBlock] = true;
...
//      return ($value > 0x7FFFFFFF) ? $value - 0x100000000 : $value;
    ...
    protected function readData($bl)
    {
        ...
        while ($block !== - 2) {
            ...
            if (isset($visited[$block]) || $block < 0) {
...
//      echo " version=".base_convert($version,10,16)
//      echo " substreamType=".base_convert($substreamType,10,16).""."\n";
...
//                  echo "\nType.XF code=".$indexCode
//                      ." dateFormat=".$this->dateFormats[ $indexCode ]
//                      ." numberFormats=".$this->numberFormats[ $indexCode ].PHP_EOL;
                ...
                case self::TYPE_DIMENSION:
                    ...
                    if (!isset($this->numRows)) {
                        if (( $length === 10 ) || ( $version === self::BIFF7 )) {
                            $this->sheets[ $this->sn ]['numRows'] = ord($this->data[ $spos + 2 ]);
                            $this->sheets[ $this->sn ]['numRows'] |= ord($this->data[ $spos + 3 ]) << 8;
                            $this->sheets[ $this->sn ]['numCols'] = ord($this->data[ $spos + 6 ]);
                            $this->sheets[ $this->sn ]['numCols'] |= ord($this->data[ $spos + 7 ]) << 8;
                        } else {
                            $this->sheets[ $this->sn ]['numRows'] = ord($this->data[ $spos + 4 ]);
                            $this->sheets[ $this->sn ]['numRows'] |= ord($this->data[ $spos + 5 ]) << 8;
                            $this->sheets[ $this->sn ]['numCols'] = ord($this->data[ $spos + 10 ]);
                            $this->sheets[ $this->sn ]['numCols'] |= ord($this->data[ $spos + 11 ]) << 8;
                        }
                    }
